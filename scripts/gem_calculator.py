...
def get_gem_price(base_price: float, level: int) -> float:
    ...
    Returns:
        float: 宝石价格
    """
    assert base_price > 0. and 0 < level < 21
    
    if level > 19:
        return 2 * get_gem_price(base_price, 19) + get_gem_price(base_price, 17) + 2 * get_gem_price(base_price, 18)
    if level > 18:
        return 2 * get_gem_price(base_price, 18) + get_gem_price(base_price, 15) + get_gem_price(base_price, 16) + get_gem_price(base_price, 17)
    if level > 17:
        return 2 * get_gem_price(base_price, 17) + get_gem_price(base_price, 13) + get_gem_price(base_price, 14) + get_gem_price(base_price, 16)
    if level > 16:
        return 2 * get_gem_price(base_price, 16) + get_gem_price(base_price, 15)
    if level > 15:
        return 2 * get_gem_price(base_price, 15) + get_gem_price(base_price, 11) + get_gem_price(base_price, 12) + get_gem_price(base_price, 13)
    if level > 14:
        return 2 * get_gem_price(base_price, 14) + get_gem_price(base_price, 9) + get_gem_price(base_price, 12)
    if level > 13:
        return 2 * get_gem_price(base_price, 13) + get_gem_price(base_price, 9) + get_gem_price(base_price, 10)
    if level > 12:
        return 2 * get_gem_price(base_price, 12) + get_gem_price(base_price, 9)
    if level > 11:
        return 2 * get_gem_price(base_price, 11) + get_gem_price(base_price, 3) + get_gem_price(base_price, 5) + get_gem_price(base_price, 6)
    
    return base_price * 2 ** (level - 1)
...
def get_star_stone_price(base_price: float, level: int) -> float:
    ...
    Returns:
        float: 星辉石价格
    """
    assert base_price > 0. and 0 < level < 12
    
    if level > 10:
        return 3 * get_star_stone_price(base_price, 10) + get_star_stone_price(base_price, 9)
    if level > 9:
        return 3 * get_star_stone_price(base_price, 9) + get_star_stone_price(base_price, 6) + get_star_stone_price(base_price, 7)
    if level > 8:
        return 3 * get_star_stone_price(base_price, 8) + get_star_stone_price(base_price, 5)
    
    return base_price * 3 ** (level - 1)
...
def get_spirit_dust_price(base_price: float, level: int) -> float:
    ...
    sqrt2 = sqrt(2)
    return base_price / (2 * sqrt2) * ((1 + sqrt2) ** level - (1 - sqrt2) ** level)
